...
function ControllAddTicker(props) {
	...
	content = <div className="button"><a href="#" className="myButton" onClick={(e) => {setIsOpen(true);}}>add ticker</a></div>
	if(modalIsOpen) {
		modalContent =  <div className="tickers-add-ticker">
							{tickers.map((value, index) => {
								if(!value.visible) {
									_show++;
									
									if(showingTicker.includes(value.ticker)) {
										return (<div key={value.ticker} className="tickers-add-ticker-wait">
											<div><span>wait adding</span></div>
											<div className="tickers-add-ticker-name-w"><span>{value.ticker}</span></div>
											<div><span>...</span></div>
										</div>);
									} else {
										return (<div key={value.ticker} className="tickers-add-ticker-item">
											...
											<div className="tickers-add-ticker-button"><a href="#" className="myButton" onClick={(e) => {
